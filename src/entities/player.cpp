...
void Player::Update(float deltaTime) {
    ...
    // Special case: Salvia at peak inverts the view with negative FOV
    ...
    if (onSalviaPeak) {
        // Negative FOV for inverted/upside-down view during entire Salvia peak
    ...
    // DEBUG: Instant death with period key
    if (IsKeyPressed(KEY_PERIOD)) {
        TraceLog(LOG_INFO, "DEBUG: Instant death triggered");
        insanityManager.OnKill();  // Add trauma to boost insanity
        insanityManager.OnKill();  // Stack it
        insanityManager.OnKill();  // Stack it
        insanityManager.OnKill();  // Stack it
        insanityManager.OnKill();  // Stack it (5 kills = 1.0 insanity)
    }
...
void Player::HandleUseItem() {
    ...
    if (itemType.find("weapon") != std::string::npos) {
        ...
        if (hitPerson) {
            ...
            // Increase player's insanity from killing
            OnKillPerson();
...
void Player::OnKillPerson() {
    insanityManager.OnKill();
}
